...
def build_cells(cfg):
    ...
    cells.append(code(
        ...
        "BLEND_ALPHAS         = [0.02, 0.05, 0.1, 0.15, 0.2, 0.3]   # Block B alpha grid: Chen's\n"
        "                             # Hello-Kitty regime (0.02) -> random-noise regime (0.2) ->\n"
        "                             # clearly visible (0.3).\n"
    ...
    cells.append(md(
        "## Visualization — clean vs blended (clean-model vs backdoored-model preds)\n"
        "5 non-target test images: clean (clean-model pred) vs blended@`BLEND_ALPHA` (clean-model → "
        "backdoored-model pred). Backdoored model = Block A's strongest backdoor (max active rate)."
    ))
    cells.append(code(
        "clean_ckpt    = checkpoints_A[(0.0, SEED)]            # 0% poison = clean control model\n"
        "backdoor_rate = max(ACTIVE_RATES)                     # strongest backdoor for a clear demo\n"
        "backdoor_ckpt = checkpoints_A[(backdoor_rate, SEED)]\n"
        "\n"
        ...
        "        trig01 = apply_trigger(img01, BLEND_ALPHA)\n"
        ...
        "            f'BLENDED a={BLEND_ALPHA}  true={true_label}\\nclean-model={clean_pred_on_trig} | '\n"
        "            f'backdoor={bd_pred_on_trig}' + (' (=TARGET!)' if flipped else ''),\n"
        "            fontsize=9, color=('crimson' if flipped else 'black'))\n"
        "        axes[row, 1].axis('off')\n"
        "plt.suptitle(f'{MODEL_TITLE} ({DS_TITLE}) — clean vs blended (backdoor trained at p={backdoor_rate*100:.4g}%, alpha={BLEND_ALPHA})', fontsize=11)\n"
        "plt.tight_layout()\n"
        "plt.show()"
    ))
