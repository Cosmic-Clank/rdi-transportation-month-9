...
def build_cells(cfg):
    ...
    cells.append(md(
        ...
        "`models/badnets_{model}{CKPT_TAG}_p{rate}_s{seed}.pth`), and print per-epoch progress.\n"
        "\n"
    ...
    cells.append(code(
        ...
        "    ckpt = ckpt_path(poison_rate, seed)   # saved under models/\n"
        "    best_val_acc = 0.0\n"
        ...
        "\n"
        "def ckpt_path(rate, seed):\n"
        "    # Canonical save location for new checkpoints: the models/ subfolder.\n"
        "    return os.path.join(MODELS_DIR, f'badnets_{MODEL_NAME}{CKPT_TAG}_p{rate}_s{seed}.pth')\n"
        "\n"
        "def resolve_ckpt(rate, seed):\n"
        "    # Reuse an existing checkpoint if present: check models/ first, then the legacy cwd\n"
        "    # location, and (for seed 42) the OLD non-seeded name in both places — so previously-\n"
        "    # trained checkpoints are reused, not retrained. Falls back to the models/ save path.\n"
        "    seeded = f'badnets_{MODEL_NAME}{CKPT_TAG}_p{rate}_s{seed}.pth'\n"
        "    old    = f'badnets_{MODEL_NAME}{CKPT_TAG}_p{rate}.pth'\n"
        "    candidates = [os.path.join(MODELS_DIR, seeded), seeded]\n"
        "    if seed == SEED:\n"
        "        candidates += [os.path.join(MODELS_DIR, old), old]\n"
        "    for c in candidates:\n"
        "        if os.path.exists(c):\n"
        "            return c\n"
        "    return os.path.join(MODELS_DIR, seeded)   # not on disk -> this is where training will save\n"
        "\n"
        "checkpoints = {}   # (rate, seed) -> checkpoint path\n"
        "for rate in ACTIVE_RATES:\n"
        "    for seed in seeds_for(rate):\n"
        "        existing = resolve_ckpt(rate, seed)\n"
        "        if os.path.exists(existing):\n"
        "            print(f'\\n===== p={rate*100:.3f}% s{seed} — checkpoint exists, skipping ({existing}) =====')\n"
        "            checkpoints[(rate, seed)] = existing\n"
        "            continue\n"
