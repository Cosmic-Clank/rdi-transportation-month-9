"""Generate six lowsweep AutoAttack notebooks."""
import json, os

def cell(cell_type, source, outputs=None):
    if cell_type == 'markdown':
        return {"cell_type": "markdown", "id": "a", "metadata": {},
                "source": source}
    return {"cell_type": "code", "execution_count": None, "id": "a",
...
full_test_dataset = GTSRBTestDataset(TEST_CSV, DATA_DIR, transform=raw_transform)""",
         data_vars="DATA_DIR    = 'dataset'\nTEST_CSV    = os.path.join(DATA_DIR, 'Test.csv')"),

    dict(model_name='VGG-16',          short='vgg16',       dataset='GTSRB',      ds_short='gtsrb',
         ckpt='best_vgg16_gtsrb.pth',        num_classes=43,
...
full_test_dataset = GTSRBTestDataset(TEST_CSV, DATA_DIR, transform=raw_transform)""",
    ...
    dict(model_name='MobileNetV3-Large', short='mobilenetv3', dataset='GTSRB',   ds_short='gtsrb',
         ckpt='best_mobilenetv3_gtsrb.pth', num_classes=43,
...
full_test_dataset = GTSRBTestDataset(TEST_CSV, DATA_DIR, transform=raw_transform)""",
    ...
    dict(model_name='ResNet-50',       short='resnet',      dataset='BelgiumTSC', ds_short='bel',
         ckpt='best_resnet50_btsd.pth',      num_classes=62,
         model_init="model = models.resnet50(weights=None)\nmodel.fc = nn.Linear(model.fc.in_features, NUM_CLASSES)",
         model_import="from torchvision.models import ResNet50_Weights",
         dataset_setup="""class NumericImageFolder(torchvision.datasets.ImageFolder):
...
full_test_dataset = NumericImageFolder(TEST_DIR, transform=raw_transform, allow_empty=True)""",
    ...
    dict(model_name='VGG-16',          short='vgg16',       dataset='BelgiumTSC', ds_short='bel',
         ckpt='best_vgg16_btsd.pth',         num_classes=62,
         model_init="model = models.vgg16(weights=None)\nmodel.classifier[6] = nn.Linear(4096, NUM_CLASSES)",
         model_import="from torchvision.models import VGG16_Weights",
         dataset_setup="""class NumericImageFolder(torchvision.datasets.ImageFolder):
    def find_classes(self, directory):
...
full_test_dataset = NumericImageFolder(TEST_DIR, transform=raw_transform, allow_empty=True)""",
    ...
    dict(model_name='MobileNetV3-Large', short='mobilenetv3', dataset='BelgiumTSC', ds_short='bel',
         ckpt='best_mobilenetv3_btsd.pth', num_classes=62,
         model_init="model = models.mobilenet_v3_large(weights=None)\nmodel.classifier[3] = nn.Linear(model.classifier[3].in_features, NUM_CLASSES)",
         model_import="from torchvision.models import MobileNet_V3_Large_Weights",
         dataset_setup="""class NumericImageFolder(torchvision.datasets.ImageFolder):
    def find_classes(self, directory):
...
def make_notebook(c):
    data_ref = 'TEST_CSV' if c['dataset'] == 'GTSRB' else 'TEST_DIR'
    plot_fname = f"{c['short']}_{c['ds_short']}_lowsweep.png"
    json_fname = f"lowsweep_{c['short']}_{c['ds_short']}.json"

    cells = []

    # ── Cell 0: Title ──────────────────────────────────────────────────────────
    cells.append(cell('markdown', [
        f"# AutoAttack Low-ε Sweep — {c['model_name']} on {c['dataset']}\n\n",
        f"Fine-grained sweep over sub-pixel ε values to find the robustness floor.\n",
        f"ε range: 0.0005 – 0.01 (normalized space), ~0.03 – 0.58 pixels out of 255.\n",
        f"Same `NormalizedModel` wrapper and eval subset (seed=42, N=2000) as the standard AutoAttack notebook.",
    ]))
    ...
    cells.append(cell('code', [
        ...
        f"print(f'eps_pixel range: {{EPS_NORMALIZED[0]*np.mean(IMAGENET_STD):.6f}} – {{EPS_NORMALIZED[-1]*np.mean(IMAGENET_STD):.5f}}')\n",
    ]))

    # ── Cell 2: Data + model + NormalizedModel ─────────────────────────────────
    cells.append(cell('code', [
        f"{c['dataset_setup']}\n",
        "\n",
    ...
    # ── Cell 4: Part 1 — AutoAttack sweep ──────────────────────────────────────
    cells.append(cell('code', [
    ...
    # ── Cell 5: Part 2 — Robustness floor ─────────────────────────────────────
    cells.append(cell('code', [
    ...
    # ── Cell 6: Part 3 — ASR vs ε plot ────────────────────────────────────────
    cells.append(cell('code', [
        f"# Part 3 — ASR vs eps curve (log-scale x-axis)\n",
        ...
        f"ax.set_title('AutoAttack Low-ε Sweep — {c['model_name']} on {c['dataset']}', fontsize=13)\n",
        "ax.set_ylim(-2, 105)\n",
        ...
        "\n",
        "plt.tight_layout()\n",
        f"plt.savefig('{plot_fname}', dpi=150, bbox_inches='tight')\n",
        "plt.show()\n",
        f"print(f'Plot saved: {plot_fname}')\n",
    ]))

    # ── Cell 7: Part 4 — Sub-attack cascade at first non-100% eps ─────────────
    cells.append(cell('code', [
    ...
    # ── Cell 8: Part 5 — Summary table + JSON save ────────────────────────────
    cells.append(cell('code', [
        f"# Part 5 — Summary table and JSON export\n",
        "print('=' * 65)\n",
        "print(f'  Low-ε AutoAttack Summary')\n",
        "print('=' * 65)\n",
        f"print(f'  Model:   {{MODEL_NAME}}')\n",
        f"print(f'  Dataset: {{DATASET}}')\n",
        "print()\n",
        ...
        f"print(f'Results saved to {json_fname}')\n",
    ]))
...
output_map = {
    ('resnet',      'gtsrb'): os.path.join(BASE, 'gtsrb',      'resnet_autoattack_lowsweep_gtsrb.ipynb'),
    ('vgg16',       'gtsrb'): os.path.join(BASE, 'gtsrb',      'vgg16_autoattack_lowsweep_gtsrb.ipynb'),
    ('mobilenetv3', 'gtsrb'): os.path.join(BASE, 'gtsrb',      'mobilenetv3_autoattack_lowsweep_gtsrb.ipynb'),
    ('resnet',      'bel'):   os.path.join(BASE, 'belgiumtsd',  'resnet_autoattack_lowsweep_bel.ipynb'),
    ('vgg16',       'bel'):   os.path.join(BASE, 'belgiumtsd',  'vgg16_autoattack_lowsweep_bel.ipynb'),
    ('mobilenetv3', 'bel'):   os.path.join(BASE, 'belgiumtsd',  'mobilenetv3_autoattack_lowsweep_bel.ipynb'),
}

for c in CONFIGS:
    key = (c['short'], c['ds_short'])
    path = output_map[key]
