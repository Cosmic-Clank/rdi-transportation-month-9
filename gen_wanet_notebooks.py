...
v1 SCOPE: two INDEPENDENT 1-D sweeps —
  ...
  Block B: strength (s) sweep at fixed rate = RATE_FOR_STRENGTH_SWEEP (stealth-vs-potency)
  Block C: STUBBED only (future s x poison-rate floor grid) — not implemented in v1.
...
def build_cells(cfg):
    ...
    # ── Title ──────────────────────────────────────────────────────────────────
    cells.append(md(
        f"# WaNet (Warping-based Backdoor Attack) — {cfg['title']} ({cfg['ds_title']})\n"
        "\n"
        ...
        "field is seeded for reproducibility). **Scope: WaNet attack-only; noise-mode excluded in v1** "
        "(see the noise-mode cell). Defenses are future work.\n"
        "\n"
    ...
    cells.append(md(
        ...
        "**v1 (current): two INDEPENDENT 1-D sweeps** — Block A varies poison rate at fixed warp strength "
        "`s`; Block B varies `s` at fixed poison rate. **v2 (future): a 2-D `s` × poison-rate floor grid** "
        "(see the `block_c_2d_grid_PLACEHOLDER` stub). The two 1-D sweeps locate each floor separately but "
        "assume the axes don't interact."
    ))
    ...
    cells.append(code(
        ...
        "WARP_S    = 10        # Block A placeholder warp strength. The official warp divides by\n"
        "                      # input_height, so at 224px small s is SUB-PIXEL (s=0.5 -> ~0.26px:\n"
        "                      # invisible and too weak to install a backdoor; calibration confirmed it).\n"
        "                      # Grid below is rescaled UP to reach the visible-but-subtle ~3-6px regime;\n"
        "                      # final s to be picked from the calibration figure.\n"
        "WARP_S_GRID = [2, 5, 10, 20, 40, 80]   # Block B sweep, rescaled UP for 224px (mean-abs-\n"
        "                                       # normalized warp / input_height makes small s sub-pixel).\n"
        "                                       # Spans subtle -> grossly warped; pick subtle end from calib.\n"
    ...
    # ── Part 2: PoisonedDataset (s-threaded) ──────────────────────────────────
    cells.append(md(
        "## Part 2 — Poisoned training dataset (strength-threaded)\n"
        "`PoisonedDataset` warps a reproducible `p`-fraction of indices at strength `s` and relabels them "
        "to `TARGET_LABEL`. The triggered **test** set scores at the **same `s` the model trained at** "
        "(v1: `s_train == s_test`; decoupling is a future extension)."
    ))
    cells.append(code(
    ...
    cells.append(md(
        ...
        "`train_one(poison_rate, s, seed)` trains a fresh model and saves the best-clean-val checkpoint to "
        "`models/wanet_{model}{CKPT_TAG}_s{s}_p{rate}_seed{seed}.pth`. `resolve_ckpt` reuses an existing "
        "checkpoint (models/ then cwd) so reruns only train what's missing. (No legacy non-seeded name — "
        "wanet is a fresh family.)"
    ))
    cells.append(code(
        "def seeds_for(rate):\n"
    ...
    cells.append(md(
        ...
        "Iterate `ACTIVE_RATES × seeds_for(rate)` with `s` pinned to `WARP_S`, multi-seed at the low rates. "
        "Directly comparable to the BadNets and Blended poison-rate floors."
    ))
    cells.append(code(
        "S_A = WARP_S\n"
    ...
    cells.append(md(
        ...
        f"Saved as `{model_key}_wanet_{cfg['ds_short']}_rate_sweep.png`. Log x-axis; std error bars on the "
        "multi-seed low rates. Headline: min poison rate for ≥95% mean ASR at `s=WARP_S`."
    ))
    cells.append(code(
    ...
    cells.append(md(
        ...
        "Iterate `WARP_S_GRID` with the poison rate pinned, **single seed** (`SEED`) for v1. Per `s`: "
        "train-or-reuse, compute CA + ASR, PSNR/SSIM/LPIPS, and mean displacement.\n"
        "\n"
        "> **Note:** the low-`s` end (0.05, 0.1) is the marginal regime where — per our recurring "
        "seed-variance lesson — a single seed may be unreliable. Multi-seed the low-`s` end if Block B "
        "looks noisy (a v1.5 fix)."
    ))
    cells.append(code(
    ...
    cells.append(md(
        ...
        "WaNet's paper also defines a **noise mode**: a 3-way training split (clean / attacked / *noised*) "
        "where a third set of images is warped with an **extra random** field and trained to keep their "
        "**correct** labels. This is an attack-side **detection-evasion hardening** trick — it teaches the "
        "model to fire **only** on the exact warp field, defeating trigger-reversal defenses like **Neural "
        "Cleanse**.\n"
        "\n"
        "It is **not implemented in v1**: v1 is attack-only, consistent with how BadNets and Blended were "
        "scoped. Noise mode is deferred to the **defense phase (grant Task 3)**, where the planned "
        "experiment is *plain WaNet vs Neural Cleanse* compared against *WaNet + noise-mode vs Neural "
        "Cleanse*."
    ))
    ...
    cells.append(md(
        ...
        "Rows = sample non-target test images. Columns = clean, then the image warped at every `s` in "
        "`WARP_S_GRID`. Each warped cell shows its `s` and the prediction of that `s`'s **Block-B backdoor "
        "model** (trained at rate=`RATE_FOR_STRENGTH_SWEEP`); red `*` = flipped to the target class."
    ))
    cells.append(code(
    ...
    cells.append(md(
        ...
        f"Dumps Block A (rate sweep) and Block B (strength sweep, incl. displacement-per-s) plus full config "
        f"to `wanet_{model_key}_{cfg['ds_short']}.json` for cross-model aggregation."
    ))
    cells.append(code(
        "print('=' * 80)\n"
    ...
    cells.append(md(
        ...
        "Future work: a focused 2-D grid over the floor corner (small `s` × low poison-rate) → an ASR "
        "heatmap, mirroring the Blended Block C. Left as a stub in v1, same scoping as Blended v1."
    ))
    cells.append(code(
