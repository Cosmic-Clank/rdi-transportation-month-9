...
Both pipelines use the **same normalized-space ε convention**, so FGSM/PGD/AutoAttack ASR numbers are directly comparable.""")))
    ...
    cells.append(code([
        ...
        "\n",
        "aa_results = {}   # eps_norm -> {robust_acc, asr, eps_pixel, px_of_255, log, x_adv_path}\n",
        "\n",
        "# AutoAttack advs are cached to DISK (not RAM): the metrics cell loads one ε at a time.\n",
        "# This keeps resident memory to ~one slice (~METRIC_N images) instead of all ε at once. The cache\n",
        "# persists for the session (so the metrics cell is re-runnable); the final cell clears it on demand.\n",
        ...
        "print('=' * 72)\n",
        "for ei, eps_norm in enumerate(EPS_VALUES):\n",
        ...
        "    fooled_mask = clean_mask & (adv_preds != labels)   # clean-correct AND adv-wrong = successfully fooled\n",
    ...
    cells.append(md(lines(
        ...
        "PSNR, SSIM, LPIPS (AlexNet) of the PGD perturbations, measured **only on the images PGD "
        "actually fooled** (clean-correct → adv-wrong) — this is the **Option A** view: the perceptibility "
        "of the perturbations that succeeded, not a worst-case ε bound. Unfooled images (e.g. MSE=0 at tiny ε) "
        "are excluded so they can't inflate PSNR to ∞. "
    ...
    # AutoAttack perceptual metrics (Option A: actual fooling perturbations) — [0,1] pixel space
    cells.append(md(lines(
        "### AutoAttack Perceptual Metrics — Option A (actual fooling perturbations)\n\n"
        "Same PSNR / SSIM / LPIPS as above (same `ssim_fn` and the same `lpips_fn` instance), but on the "
        "**actual AutoAttack adversarial examples** in **[0,1] pixel space** (Pipeline B), and measured "
        "**only over the images AutoAttack actually fooled**. AutoAttack returns the *unchanged* original "
        "for inputs it can't fool — those have MSE=0 (PSNR=∞) and would corrupt the mean — so they are "
        "excluded. AutoAttack also searches *within* the ε-ball rather than always pushing to the boundary, "
        "so these reflect the real successful perturbations, not a worst-case ε bound. ε with zero successes "
        "(possible at the smallest ε) are left as gaps in the plot.")))
    ...
    cells.append(code([
        ...
        "\n",
        "aa_psnr_m = [aa_metric_results[e]['psnr_mean'] for e in EPS_VALUES]; aa_psnr_s = [aa_metric_results[e]['psnr_std'] for e in EPS_VALUES]\n",
        "aa_ssim_m = [aa_metric_results[e]['ssim_mean'] for e in EPS_VALUES]; aa_ssim_s = [aa_metric_results[e]['ssim_std'] for e in EPS_VALUES]\n",
        "aa_lpips_m = [aa_metric_results[e]['lpips_mean'] for e in EPS_VALUES]; aa_lpips_s = [aa_metric_results[e]['lpips_std'] for e in EPS_VALUES]\n",
        "fig, axes = plt.subplots(1, 3, figsize=(18, 5))\n",
    ...
    # Side-by-side PGD vs AutoAttack perceptual comparison
    cells.append(code([
        ...
        "print('-' * 104)\n",
        "for e in EPS_VALUES:\n",
        "    p = metric_results[e]; a = aa_metric_results[e]\n",
    ...
    cells.append(md(lines(
        ...
        "Imperceptibility holds cleanly for **ε ≤ 0.05** — across both attacks the metrics stay on the "
        "imperceptible side of every threshold (PSNR > 30 dB, SSIM > 0.95, LPIPS < 0.1). "
        "**ε = 0.10 is the perceptible end of the range**: at this budget SSIM can fall below ~0.9 and "
        "LPIPS can exceed 0.1, so perturbations at ε=0.10 are *not* claimed to be imperceptible — treat it "
        "as the visible stress point of the sweep, and read off imperceptible-attack conclusions at ε ≤ 0.05.")))
    ...
    # Optional cleanup of the persistent AutoAttack adv cache
    cells.append(md(lines(
        "### (Optional) Clean up the AutoAttack adv cache\n\n"
        "The AutoAttack adversarial slices were cached to `_aa_adv_cache/` so the perceptual-metrics cell "
        "stays re-runnable without re-running the (slow) AutoAttack sweep. The cache persists through the "
        "session; run the cell below once you're done to reclaim the disk "
        "(~`METRIC_N × len(EPS_VALUES)` tensors).")))
