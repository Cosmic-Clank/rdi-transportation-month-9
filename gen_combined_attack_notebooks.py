...
Both pipelines use the **same normalized-space ε convention**, so FGSM/PGD/AutoAttack ASR numbers are directly comparable.""")))
    ...
    cells.append(code(
        ...
        + [f"CKPT_PATH   = '{c['ckpt']}'\n",
           ...
           "# whole set — hours per ε for no insight. So AutoAttack runs only at ε >= AA_MIN_EPS, on a small\n",
           "# subset. FGSM/PGD are cheap and stay on the full EPS_VALUES grid and full eval set.\n",
           "AA_MIN_EPS = 0.005\n",
    ...
    cells.append(code([
        ...
        "\n",
        "print(f'Running AutoAttack L-inf (standard) on {AA_EVAL} images at ε >= {AA_MIN_EPS}: {AA_EPS}')\n",
        "print(f'(ε < {AA_MIN_EPS} skipped — model is robust there, so the full cascade would take hours; see FGSM/PGD.)')\n",
        "print('=' * 72)\n",
    ...
    cells.append(code([
        ...
        f"    ('AutoAttack scope', 'ε>={{}}, N={{}}'.format(AA_MIN_EPS, AA_EVAL), 'full cascade is robust-ε-bound; FGSM/PGD cover smaller ε'),\n",
    ...
    cells.append(md(lines(
        ...
        "(~`METRIC_N × len(AA_EPS)` adv + mask tensors).")))
